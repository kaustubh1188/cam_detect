...
def main():
    ...
    st.sidebar.markdown("Developed by Raviteja Katkam")
    ...
    if input_type == "Image":
        ...
        ip_address = st.text_input("Enter the IP camera stream URL (e.g., rtsp://... or http://...):")
        start_button = st.button("Start Stream")

        if start_button and ip_address:
            # Set up video capture from the IP camera stream
            cap = cv2.VideoCapture(ip_address)

            if not cap.isOpened():
                st.error("Error: Could not open the IP camera stream.")
                return
            ...
            while cap.isOpened():
                ...
                frame, prediction_text = process_frame(model, frame, conf_threshold, iou_threshold)
